...
<head>
    ...
    <link rel="stylesheet" href="css/deposito_bancario.css" />
	<!-- Scripts-->
    ...
    <script src="js/egreso_bancario.js" type="text/javascript"></script>
...
<body>
	...
	<?php require('../../menu_principal.php'); ?>
		...
		<section class="container contenido_cuentas">
      	<h4 class="titulo_ventana">Egresos Bancarios</h4>
...
$sql="SELECT b.*,e.* from ban_egreso_bancario e INNER JOIN banco b ON b.ban_id=e.egre_bancoid";
$res = $link->query($sql);
while($data=$res->fetch_assoc())
{
    $codigo=$data["egre_id"];
       ...
       $act_print = ' <a title="Imprimir" class="" style="color:#337ab7;background-color: transparent;border: none;" href="#" onclick="ver_egreso('.$codigo.')"><span class="glyphicon glyphicon-zoom-in"></span></a>';
     echo"
    <tr id='row_$codigo'>
    <td>$data[egre_numegreso]</td>
    <td>$data[egre_fecha]</td>
    <td>$data[ban_nombre]</td>
    <td>$data[ban_codigo]</td>
    <td>$data[ban_numero_cuenta]</td>
    <td>$data[ban_tipo]</td>
    <td>$data[egre_descripcion]</td>
       ...
       <div class="modal bs-example-modal-lg fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
		...
		<div class="modal-content">
            ...
            <input  type="hidden" id="filas_diario" name="filas_diario"  />
             <input type="hidden" name="identificador_egreso" id="identificador_egreso" value=""/>
           <form id="NuevoDeposito" name="NuevoDeposito" method="post" class="form-horizontal">
           <div class="modal-header">
              <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
              <h4 class="modal-title">Agregar Egreso Bancario</h4>
           </div>
           <div class="modal-body">
           <!-- INPUT DEPOSITO BANCARIO Y FECHA-->
             <div class="form-group box_banco " >
             <div class="rowContainer">
                 <label class="col-xs-2 control-label" style="margin-left:-30px;">Egre # </label>
                <div class="col-xs-2">
                    <input data-fv-row=".rowContainer"  type="text" class="form-control input-sm" name="numero_egreso" id="numero_egreso" value="<?php //echo $cod; ?>"  onkeydown="return false" />
                </div>
             </div>
          ...
          <div class="rowContainer">
          <label class="col-xs-2 control-label"># de Cheque</label>
          <div class="col-xs-2">
          <input data-fv-row=".rowContainer" value=""  type="text" class="form-control input-sm" name="num_cheque" id="num_cheque" onkeydown="return false"  />
          </div>
          </div>
          <div class="rowContainer">    
          ...
          <div class="col-xs-3">
          <input data-fv-row=".rowContainer" value=""  type="text" class="form-control input-sm" name="beneficiario" id="beneficiario" />
          </div>
          </div>
          ...
          <div class="col-xs-3">
            <select data-fv-row=".rowContainer" name="nombre_proveedor" id="nombre_proveedor" class="selectpicker form-control input-sm" data-live-search="true" data-width="200px">
                 <option value=" " >Seleccione una Opci&oacute;n</option>
                    <?php
                    /** LLENA EL SELECT CON DATOS DE LA TABLA PLAN DE CUENTAS */ 
        ...
        <div class="modal-footer" style="margin-top: 10px;">
         <div class="form-group">
            <div class="col-xs-5 col-xs-offset-3">
                ...
                <button type="button" class="btn btn-primary" id="imprimir_egreso" name="imprimir_egreso" style="display: none;">Imprimir</button>
