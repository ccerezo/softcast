...
<html>
<head>
	<title>Transferencias Recibidas</title>
    ...
    <script src="js/transferencias_recibidas.js" type="text/javascript"></script>
...
$sql="SELECT b.*,c.*,rec.* from ban_transferencia_recibida rec INNER JOIN banco b ON b.ban_id=rec.tra_rec_bancoid
INNER JOIN cliente c ON c.id=rec.tra_rec_clienteid";
$res = $link->query($sql);
while($data=$res->fetch_assoc())
{
    $codigo=$data["tra_rec_id"];
     ...
     echo"
    <tr id='row_$codigo'>
    <td>$data[tra_rec_numtransferencia]</td>
    <td>$data[tra_rec_fecha]</td>
    <td>$data[ban_nombre]</td>
    <td>$data[ban_codigo]</td>
    <td>$data[ban_numero_cuenta]</td>
    <td>$data[ban_tipo]</td>
    <td>$data[tra_rec_descripcion]</td>
       ...
       <div class="modal bs-example-modal-lg fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
		...
		<div class="modal-content">
           ...
           <div class="modal-header">
              ...
              <h4 class="modal-title">Agregar Transferencia Recibida</h4>
           </div>
