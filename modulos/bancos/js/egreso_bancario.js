                    ...
                    if(data!=='fallo'){
                        ...
                        bootbox.alert({
                            ...
                            callback: function() {
                               ...
                               $("#imprimir_egreso").show();

                               $("input").attr('disabled', true);
                              
                               $(".selectpicker").attr('disabled', true);
                                   $("#identificador_egreso").val(response[0]["id_egreso"]);
                            }
                        });
                    }else{
                        bootbox.alert({
                            title: "Información",
                            message: "El egreso no se agregó Revise los datos y vuelva a intentar!",
                            buttons: {
                                'ok': {
                                    label: 'Aceptar',
...
/***** DA CONFLICTOS CON EL BOOTSTRAP SELECT***/
  ...
  $("#imprimir_egreso").click(function(){
        var id = $("#identificador_egreso").val();
        ver_egreso(id);
    });              
...
function cambiar_numero_asiento(){
    ...
    if(texto===""){
        ...
        aux1 = $("#numero_egreso").val().substring(3,10);
        texto = texto.replace(aux1, aux2);
        aux3 = $("#numero_egreso").val().substring(11,15);
    }   
      console.log(aux2+aux3+" "+texto+" "+aux1);
        $.ajax({
            ...
            type: "post",
            data:{
                proceso: 'ultimo_diario_mes_egreso',
                mes: aux2
            },
            success: function (data) {
              console.log(data);
...
function cambiar_numero_cheque(){
    var texto =$("#nombre_banco").val();
      ...
      //  cambiar_numero_cheque();
    $('#nombre_banco').on('change', function(){
        /* selectedOption = $('option:selected', this);
     
          
            $('input[name=hidden_banco_id]').val( selectedOption.data('id') );
            
            
            if(selectedOption=='')
                 $('input[name=hidden_banco_id]').val('');*/
         cambiar_numero_cheque();
     });
...
//Fin del document ready
  ...
  function ver_egreso(id){ 
    ...
    var WindowObject = window.open(url,"Reporte Egresos","width="+w+",height="+h+",scrollbars=yes,top="+y+", left="+x+"") 
    WindowObject.document.title = "Reporte Egresos";
}
