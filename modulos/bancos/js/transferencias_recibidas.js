    ...
    .on('change', 'input[name="fecha"]', function(e) {
       ...
       $("#filas_diario").val(length);  
         ...
         $.ajax({
                ...
                type: "post",
                data: {
                    proceso: "ingresar_trans_recibida",
                    filas: length,
                    datos: datos
                },
                 beforeSend : function(){
                ...
                success: function(data){
                    ...
                    if(data!=='fallo'){
                        ...
                        bootbox.alert({
                            ...
                            message: "La Transferencia Recibida se agreg&oacute; Correctamente! ",
                            buttons: {
                                'ok': {
                                    label: 'Aceptar',
                            ...
                            callback: function() {
                               ...
                               $(".selectpicker").attr('disabled', true);
                                   $("#identificador_deposito").val(response[0]["id_recibido"]);
                            }
                        });
                    }else{
                        bootbox.alert({
                            title: "Información",
                            message: "La Transferencia Recibida no se agreg&oacute; Revise los datos y vuelva a intentar!",
                            buttons: {
                                'ok': {
                                    label: 'Aceptar',
    ...
    if(texto===""){
      ...
      console.log(aux2+aux3+" "+texto+" "+aux1);
        $.ajax({
            ...
            type: "post",
            data:{
                proceso: 'ultimo_diario_mes_trans_rec',
                mes: aux2
            },
            success: function (data) {
              console.log(data);
...
//Fin del document ready
  ...
  function ver_deposito(id){ 
    ...
    url = "reporte_trans_recibidas.php?id="+id;
    var w = 850;
    var h = 550;
    ...
    var WindowObject = window.open(url,"Reporte Transferencia Recibida","width="+w+",height="+h+",scrollbars=yes,top="+y+", left="+x+"") 
    WindowObject.document.title = "Reporte Transferencia Recibida";
}
