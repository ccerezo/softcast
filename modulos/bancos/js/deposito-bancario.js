            ...
            $.ajax({
                ...
                success: function(data){
                    ...
                    if(data!=='fallo'){
                        ...
                        bootbox.alert({
                            title: "Información",
                            message: "El Depósito Bancario. Se agregó Correctamente!",
                            buttons: {
                                'ok': {
                                    label: 'Aceptar',
                            ...
                            callback: function() {
                                ...
                                $("#identificador_bancario").val(response[0]["id_deposito"]);
                                //location.reload();
                            }
                        });
                    }else{
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            if(data==='vacio'){
                ...
                var response = JSON.parse(data);
                console.log(response);
                $("#tabla_depositos tbody").remove();
                nuevaFila+="<tbody>";
                for(var i=0; i<response.length; i++){
                    nuevaFila+="<tr id='depositos"+response[i]["id_deposito"]+"'>"; 
                    nuevaFila+="<td>"+response[i]["num_deposito"]+"</td>";
                    ...
                    nuevaFila+="<td><a style='cursor:pointer;' onclick='ver("+response[i]["id_deposito"]+")' title='Ver'><span class='glyphicon glyphicon-zoom-in'></span></a>\n\
                                    <a style='cursor:pointer;' onclick='eliminar("+response[i]["id_deposito"]+")' title='Eliminar'><span class='glyphicon glyphicon-trash'></span></a>\n\
                                    <a style='cursor:pointer;' onclick='editar("+response[i]["id_deposito"]+")' title='Editar'><span class='glyphicon glyphicon-pencil'></span></a></td>";
                    nuevaFila+="</tr>";
                }
                nuevaFila+="</tbody>";
                
                $("#tabla_depositos").append(nuevaFila);
                cambiar_numero();
            }
            $('#tabla_depositos').DataTable({
                "bLengthChange": false,
                "responsive": true,
                "language": {
