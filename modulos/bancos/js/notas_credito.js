            ...
            $.ajax({
                ...
                success: function(data){
                    ...
                    if(data!=='fallo'){
                        ...
                        bootbox.alert({
                            title: "Información",
                            message: "La Nota de Crédito. Se agregó Correctamente!",
                            buttons: {
                                'ok': {
                                    label: 'Aceptar',
                            ...
                            callback: function() {
                                ...
                                $("#identificador_bancario").val(response[0]["id_nc"]);
                                //location.reload();
                            }
                        });
                    }else{
                        bootbox.alert({
                            title: "Información",
                            message: "La Nota de Crédito. No se Agregó Revise los datos y vuelva a intentar!",
                            buttons: {
                                'ok': {
                                    label: 'Aceptar',
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            if(data==='vacio'){
                ...
                var response = JSON.parse(data);
                console.log(response);
                $("#tabla_credito tbody").remove();
                nuevaFila+="<tbody>";
                for(var i=0; i<response.length; i++){
                    nuevaFila+="<tr id='depositos"+response[i]["id_credito"]+"'>"; 
                    nuevaFila+="<td>"+response[i]["num_credito"]+"</td>";
                    ...
                    nuevaFila+="<td><a style='cursor:pointer;' onclick='ver("+response[i]["id_credito"]+")' title='Ver'><span class='glyphicon glyphicon-zoom-in'></span></a>\n\
                                    <a style='cursor:pointer;' onclick='eliminar("+response[i]["id_credito"]+")' title='Eliminar'><span class='glyphicon glyphicon-trash'></span></a>\n\
                                    <a style='cursor:pointer;' onclick='editar("+response[i]["id_credito"]+")' title='Editar'><span class='glyphicon glyphicon-pencil'></span></a></td>";
                    nuevaFila+="</tr>";
                }
                nuevaFila+="</tbody>";
                
                $("#tabla_credito").append(nuevaFila);
                cambiar_numero();
            }
...
function ver(id){ 
    ...
    url = url+"reporte-creditos.php?id="+id;
    var w = 850;
    var h = 550;
    ...
    var WindowObject = window.open(url,"Nota de Crédito","width="+w+",height="+h+",scrollbars=yes,top="+y+", left="+x+"") 
    WindowObject.document.title = "Nota de Crédito";
}
...
function cambiar_numero(){
    ...
    if(texto===""){
        ...
        $.ajax({
            ...
            type: "post",
            data:{
                orden: 'ultimo_diario_mes_credito',
                mes: aux2
            },
            success: function (data) {
                if(data!=='0'){
