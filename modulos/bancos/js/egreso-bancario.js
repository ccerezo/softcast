            ...
            $.ajax({
                ...
                success: function(data){
                    ...
                    if(data!=='fallo'){
                        ...
                        bootbox.alert({
                            title: "Información",
                            message: "El Egreso Bancario. Se agregó Correctamente!",
                            buttons: {
                                'ok': {
                                    label: 'Aceptar',
                            ...
                            callback: function() {
                                ...
                                $("#identificador_bancario").val(response[0]["id_egreso"]);
                                //location.reload();
                            }
                        });
                    }else{
                        bootbox.alert({
                            title: "Información",
                            message: "El Depósito Bancario. No se Agregó Revise los datos y vuelva a intentar!",
                            buttons: {
                                'ok': {
                                    label: 'Aceptar',
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            if(data==='vacio'){
                ...
                var response = JSON.parse(data);
                console.log(response);
                $("#tabla_egresos tbody").remove();
                nuevaFila+="<tbody>";
                for(var i=0; i<response.length; i++){
                    nuevaFila+="<tr id='depositos"+response[i]["id_egreso"]+"'>"; 
                    nuevaFila+="<td>"+response[i]["num_egreso"]+"</td>";
                    nuevaFila+="<td>"+response[i]["fecha"]+"</td>";
                    nuevaFila+="<td>"+response[i]["nombre_banco"]+" Cta."+response[i]["tipo_cuenta_banco"]+" "+response[i]["num_cuenta_banco"]+"</td>";
                    ...
                    nuevaFila+="<td><a style='cursor:pointer;' onclick='ver("+response[i]["id_egreso"]+")' title='Ver'><span class='glyphicon glyphicon-zoom-in'></span></a>\n\
                                    <a style='cursor:pointer;' onclick='eliminar("+response[i]["id_egreso"]+")' title='Eliminar'><span class='glyphicon glyphicon-trash'></span></a>\n\
                                    <a style='cursor:pointer;' onclick='editar("+response[i]["id_egreso"]+")' title='Editar'><span class='glyphicon glyphicon-pencil'></span></a></td>";
                    nuevaFila+="</tr>";
                }
                nuevaFila+="</tbody>";
                
                $("#tabla_egresos").append(nuevaFila);
                cambiar_numero();
            }
...
function ver(id){ 
    ...
    var WindowObject = window.open(url,"Egreso Bancario","width="+w+",height="+h+",scrollbars=yes,top="+y+", left="+x+"") 
    WindowObject.document.title = "Egreso Bancario";
}
...
function cambiar_numero(){
    ...
    if(texto===""){
        ...
        $.ajax({
            ...
            type: "post",
            data:{
                orden: 'ultimo_diario_mes_egreso',
                mes: aux2
            },
            success: function (data) {
                if(data!=='0'){
