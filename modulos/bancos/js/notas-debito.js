    ...
    $.ajax({
        ...
        success: function(data){
            ...
            if(data==='vacio'){
                ...
                var response = JSON.parse(data);
                $("#tabla_debito tbody").remove();
                nuevaFila+="<tbody>";
                for(var i=0; i<response.length; i++){
                    nuevaFila+="<tr id='depositos"+response[i]["id_debito"]+"'>"; 
                    nuevaFila+="<td>"+response[i]["num_debito"]+"</td>";
                    ...
                    nuevaFila+="<td><a style='cursor:pointer;' onclick='ver("+response[i]["id_debito"]+")' title='Ver'><span class='glyphicon glyphicon-zoom-in'></span></a>\n\
                                    <a style='cursor:pointer;' onclick='eliminar("+response[i]["id_debito"]+")' title='Eliminar'><span class='glyphicon glyphicon-trash'></span></a>\n\
                                    <a style='cursor:pointer;' onclick='editar("+response[i]["id_debito"]+")' title='Editar'><span class='glyphicon glyphicon-pencil'></span></a></td>";
                    nuevaFila+="</tr>";
                }
                nuevaFila+="</tbody>";
                
                $("#tabla_debito").append(nuevaFila);
                cambiar_numero();
            }
...
function ver(id){ 
    ...
    url = url+"reporte-debitos.php?id="+id;
    var w = 850;
    var h = 550;
    ...
    var WindowObject = window.open(url,"Nota de Débito","width="+w+",height="+h+",scrollbars=yes,top="+y+", left="+x+"") 
    WindowObject.document.title = "Nota de Débito";
}
...
function cambiar_numero(){
    ...
    if(texto===""){
        ...
        $.ajax({
            ...
            type: "post",
            data:{
                orden: 'ultimo_diario_mes_debito',
                mes: aux2
            },
            success: function (data) {
                if(data!=='0'){
